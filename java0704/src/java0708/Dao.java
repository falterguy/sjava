...
public class Dao {
	...
	private ResultSet rs; //데이터베이스 select(조회)결과를 받기 위한변수
	...
	public Member[] select() {
		...
		try {
			...
			if(rs.next()) {
				size=rs.getInt("cnt"); //데이터베이스에 데이터(레코드)가 총 몇개 있나?
			}
			if(size!=0) {
				list=new Member[size];
				int i=0;
				sql = "select*from test_member";//전체 데이터 조회 쿼리분(sql질의문)
				rs=st.executeQuery(sql);//조회결과 받기
				while(rs.next()) {//조회 결과 하나씩 순회
					list[i]=new Member(rs.getInt("member_id") ,
							rs.getString("name"),rs.getInt("kor"), rs.getInt("eng"), rs.getInt("mat"),rs.getInt("total"),
									rs.getInt("std_avg"));
							i++;//배열의 다음 인덱스 표현을 위해 1증거
				}
			}
...
//	public void insert(Member m) {
//		
//		String sql = "insert into test_member(name,kor,eng,mat,total,"+"std_avg) values('"+m.getName()+"',"+m.getKor()+
